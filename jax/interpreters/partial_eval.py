...
class JaxprTrace(Trace):
  ...
  def process_call(self, call_primitive, f: lu.WrappedFun, tracers, params):
    ...
    if not jaxpr.eqns:
      # Avoid staging out trivial calls
    ...
    eqn = new_eqn_recipe(in_tracers, out_tracers, call_primitive, new_params)
  ...
  def post_process_call(self, call_primitive, out_tracers, params):
    ...
    def todo(x):
      ...
      in_tracers = (*const_tracers, *env_tracers)
      update_params = call_param_updaters.get(call_primitive, dict)
      new_params = update_params(params, call_jaxpr=convert_constvars_jaxpr(jaxpr))
      eqn = new_eqn_recipe(in_tracers, out_tracers, call_primitive, new_params)
      for t in out_tracers:
  ...
  def process_map(self, map_primitive, f: lu.WrappedFun, tracers, params):
    ...
    in_pvs, in_consts = unzip2(in_pvals)
    ...
    reduced_in_pvals = map(modify_aval(core.mapped_aval),
                           zip(in_pvals, params['mapped_invars']))
    ...
    eqn = new_eqn_recipe(in_tracers, out_tracers, map_primitive, new_params)
    for t in out_tracers:
      t.recipe = eqn
    return out_tracers
